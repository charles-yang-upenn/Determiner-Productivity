...
def randomphase(chi, mot, masterlist): # only used for half and random phases

    unimw = []
    for i in range(0, len(mot)):
        nnn = mot[i].split()[1]
        if nnn != unimw and nnn in masterlist:
            if freqcounts(nnn, mot)>=2:
                unimw.append(nnn)
    uniqw = []
    for i in range(0, len(chi)):
        nnn = chi[i].split()[1]
        if nnn != uniqw and nnn in unimw and freqcounts(nnn, chi)>=2:
            uniqw.append(nnn)
    rdata = []
    for i in range(0, len(chi)):
        if chi[i].split()[1] in uniqw:
            rdata.append(chi[i])
    return rdata
...
def findmedianmean(chi, mot):
    ...
    for i in range(0, len(uniqw)):
        motf.append(freqcounts(uniqw[i], mot))
        chif.append(freqcounts(uniqw[i], chi))
        
    return numpy.median(chif), numpy.median(motf), numpy.mean(chif), numpy.mean(motf)
            
    
def main():

    # args: 1 filename 2 # of simulations in sample
    # 3: the "child" file, "CHI" for child, "MOT" for mother, or any CHILDES ID
    # 4: the "mother" file, "CHI" for CHILD, "MOT" for mother
    # 5: P: 5 phases by Pine; R: random first half; F: first half; A: all
    ...
    kid = sys.argv[1][:-4]
    chiname = kid.split('/')
    kid = chiname[len(chiname)-1]
    sim = float(sys.argv[2]) # number of simulations to draw samples

    chidata = readindata(sys.argv[1], "*"+sys.argv[3]+":")
    motdata = readindata(sys.argv[1], "*"+sys.argv[4]+":")


    if sys.argv[5] == 'P' or sys.argv[5] == 'p':
    
        start = 0 # index to the stopping point of previous phase in child data, starting at 0

        for p in range(1, 6): # phases
            ...
            print kid+'-'+sys.argv[3], p,  ("%.2f" % findbias(uniqw, childsample)),

            cosum = 0.0
            mosum = 0.0
            cnsum = 0.0
            cssum = 0.0
            for run in range(0, int(sim)): 
                tn, ts, co, mo = control(childsample, motdata)
                cosum += co
                mosum += mo
                cnsum += tn
                cssum += ts
            medianc, medianm, meanc, meanm = findmedianmean(childsample, motdata)
            
            print ("%.2f" % (cosum/sim)), ("%.2f" % (cssum/cnsum)), ("%.2f" % meanc), medianc, kid+'-'+sys.argv[4], ("%.2f" % findbias(uniqw, motdata)), ("%.2f" % (mosum/sim)), ("%.2f" % meanm), medianm

    if sys.argv[5] == 'F' or sys.argv[5] == 'f': # first 50%

        chidata = chidata[0:int(float(len(chidata))/2.0)]
        
        childsample = randomphase(chidata, motdata, masterlist)
        ...
        print kid+sys.argv[3], "Firsthalf",  ("%.2f" % findbias(uniqw, childsample)),
        ...
        for run in range(0, int(sim)): 
            ...
            cnsum += tn
            cssum += ts
        medianc, medianm, meanc, meanm = findmedian(childsample, motdata)

        print ("%.2f" % (cosum/sim)), ("%.2f" % (cssum/cnsum)), ("%.2f" % meanc), medianc, kid+sys.argv[4], ("%.2f" % findbias(uniqw, motdata)), ("%.2f" % (mosum/sim)), ("%.2f" % meanm),  medianm

    if sys.argv[5] == 'R' or sys.argv[5] == 'r': # random 50%

        tmpdata = chidata
        for i in range(0, 100): # shuffle data 100 times
            random.shuffle(tmpdata)

        chidata = []
        for i in range(0, len(tmpdata)):
            if random.random()<=0.50:
                chidata.append(tmpdata[i])
        ...
        print kid+sys.argv[3], "Randomhalf",  ("%.2f" % findbias(uniqw, childsample)),
    ...
    if sys.argv[5] == 'A' or sys.argv[5] == 'a': # all; first should be smaller 
        ...
        print kid+sys.argv[3], "All",  ("%.2f" % findbias(uniqw, childsample)),
