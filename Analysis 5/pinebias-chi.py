...
def main():
    ...
    for p in range(1, 2): # only one phase
        ...
        print chiname[len(chiname)-1], "P1", len(usedwords), ("%.2f" % b), ("%.2f" % s), ("%.2f" % B), ("%.2f" % S), '|', ("%.2f" % os), ("%.2f" % OS)
        
        
    for p in range(1, 2):  # use all  child data

        childsample = chidata
#        childsample = chidata[0:int(float(len(chidata))/2.0)]
        # childsample = []
        # for i in range(0, 100):
        #     random.shuffle(chidata)
        # for i in range(0, len(chidata)):
        #     if random.random()<=0.5:
        #         childsample.append(chidata[i])
        uniqw = []
        for i in range(0, len(childsample)):
            nnn = childsample[i].split()[1]
            if nnn not in uniqw:
        ...
        print chiname[len(chiname)-1], "ALL", len(usedwords), ("%.2f" % b), ("%.2f" % s), ("%.2f" % B), ("%.2f" % S), '|', ("%.2f" % os), ("%.2f" % OS)            
